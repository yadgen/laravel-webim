...
$d['redis'] = new Redis();
$d['redis']->connect('127.0.0.1', 6379);
$d['redis']->auth('123456');
$d['redis']->select(1);
$d['redis']->flushdb();
...
$ws_server->on('open', function ($ws, $request) use($d) {
    $d['redis']->sadd($d['swoole_fd_key'], $request->fd);

    echo "client-{$request->fd} is opened\n";
});

$ws_server->on('close', function ($ws, $fd) use($d) {
    $d['redis']->srem($d['swoole_fd_key'], $fd);

    $fd_user = $d['redis']->hgetall($d['swoole_fd_user_key']);
    $user_id = $fd_user[$fd];
    $d['redis']->hdel($d['swoole_fd_user_key'], $fd);

    $d['redis']->hdel($d['swoole_online_user_list_key'], $user_id);
...
$ws_server->on('message', function ($ws, $frame) use($d) {
    ...
    switch ($frameData['message_type']) {
        case 1: // onopen
            $d['redis']->hset($d['swoole_online_user_list_key'], $frameData['user_id'], $frameData['user_name']);
            $d['redis']->hset($d['swoole_fd_user_key'], $frame->fd, $frameData['user_id']);
            ...
            // 告诉自己，有哪些人
            $user_list = $d['redis']->hgetall($d['swoole_online_user_list_key']);
